...
// Elenco delle raccolte che vuoi caricare
const collectionsToLoad = ["reservations", "users", "admin"];

// Funzione per caricare e stampare in console tutti i documenti di una collezione
function loadCollection(collectionName) {
  db.collection(collectionName)
    .get()
    .then(querySnapshot => {
      console.log(`Documenti nella collezione ${collectionName}:`);
      querySnapshot.forEach(doc => {
        console.log(doc.id, " => ", doc.data());
      });
    })
    .catch(err => console.error(`Errore caricamento della collezione ${collectionName}:`, err));
}

// Carica tutte le raccolte specificate
collectionsToLoad.forEach(colName => loadCollection(colName));
