...
function loadAdminImagesRealtime() {
  db.collection("admin").doc("images").onSnapshot(doc => {
    ...
    if (doc.exists) {
      ...
      // Non si resetta mai: il documento viene creato una volta e poi mantenuto.
      const initialData = {};
      for (let i = 1; i <= 10; i++) {
...
function checkAndResetAfterSevenFifty() {
  ...
  const today = getTodayDate();
  const now = new Date();
  // Reset se non è già stato fatto per oggi e l'orario è maggiore o uguale a 7:50
  if (lastResetDate !== today && (now.getHours() > 7 || (now.getHours() === 7 && now.getMinutes() >= 50))) {
...
function resetAllReservations() {
  const today = getTodayDate();
  // Cancella tutte le prenotazioni con data inferiore a quella odierna (prenotazioni passate)
  db.collection("reservations")
    .where("date", "<", today)
    .get()
    .then(snapshot => {
    ...
    .then(() => {
      showNotification("Prenotazioni passate cancellate per il nuovo giorno.");
      loadReservationsFromFirestore();
    })
    .catch(err => {
