...
function authenticateStaticUser(username, password) {
  db.collection("users").doc(username).get().then(doc => {
    ...
    // Imposta i crediti di default a 0 per i nuovi utenti
    let credits = 0;
    if (doc.exists && doc.data().password) {
      ...
      if (doc.data().credits !== undefined) {
        ...
        // Se manca il campo crediti, lo inizializziamo a 0
    ...
    if (storedPassword === password) {
      currentUser = username;
      if (!doc.exists) {
        // Crea il documento dell'utente con crediti di default a 0
 ...
 ***********************/
// Contiamo TUTTE le prenotazioni per l'utente (senza filtro sulla data)
...
// In saveReservation verifichiamo i crediti: se sono > 0 si procede, altrimenti non viene eseguito l'update
...
// La funzione bookSlot controlla sia il saldo dei crediti sia il limite massimo di 2 prenotazioni totali per utente
...
function loadAdminImagesRealtime() {
  db.collection("admin").doc("images")
    .onSnapshot(doc => {
      // Recuperiamo i 4 container
      const topContainer = document.getElementById("login-images-top");
      const leftContainer = document.getElementById("login-images-left");
      const rightContainer = document.getElementById("login-images-right");
      const bottomContainer = document.getElementById("login-images-bottom");

      if (!topContainer || !leftContainer || !rightContainer || !bottomContainer) return;

      // Svuotiamo i 4 container
      topContainer.innerHTML = "";
      leftContainer.innerHTML = "";
      rightContainer.innerHTML = "";
      bottomContainer.innerHTML = "";

      if (!doc.exists) return; // Se il doc non esiste, esci

      const data = doc.data();
      const isMobile = window.innerWidth < 600; // Soglia a 600px

      if (isMobile) {
        // ============== LAYOUT MOBILE ==============
        // 10 immagini in alto, 10 in basso
        // Nascondiamo leftContainer e rightContainer
        leftContainer.style.display = "none";
        rightContainer.style.display = "none";

        // Visualizziamo top/bottom
        topContainer.style.display = "flex";
        bottomContainer.style.display = "flex";

        // Carichiamo prime 10 in top
        for (let i = 1; i <= 10; i++) {
          appendImageToContainer(data[`image${i}`], topContainer);
        }
        // Carichiamo altre 10 in bottom
        for (let i = 11; i <= 20; i++) {
          appendImageToContainer(data[`image${i}`], bottomContainer);
        }
      } else {
        // ============== LAYOUT DESKTOP ==============
        ...
        // 5 immagini a sinistra
        for (let i = 11; i <= 15; i++) {
          appendImageToContainer(data[`image${i}`], leftContainer);
        }
        // 5 immagini a destra
        for (let i = 16; i <= 20; i++) {
          appendImageToContainer(data[`image${i}`], rightContainer);
        }
      }
    });
}
// Funzione di supporto per aggiungere un'immagine ad un container
function appendImageToContainer(imgObj, container) {
...
function saveAdminImages() {
  ...
  for (let i = 1; i <= 20; i++) {
    const url = document.getElementById(`image${i}URL`)?.value.trim() || "";
    const link = document.getElementById(`image${i}Link`)?.value.trim() || "";

    // Se almeno l'URL non è vuoto, salviamo
    if (url !== "") {
      imagesData[`image${i}`] = { url, link };
    } else {
      // Se è vuoto, per pulizia potresti anche rimuoverlo:
      imagesData[`image${i}`] = firebase.firestore.FieldValue.delete();
    }
  }

  db.collection("admin").doc("images").set(imagesData, { merge: true })
    .then(() => {
