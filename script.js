...
function authenticateUser(username) {
  ...
  loadAppImagesRealtime(); // Nuova chiamata per le immagini nell'app
...
function authenticateStaticUser(username, password) {
 ...
 if (storedPassword === password) {
      ...
      loadAdminNotesRealtime();
      loadAdminImagesRealtime();
      loadAppImagesRealtime();  // Aggiungi questa riga per caricare le immagini nell'area utente
      startUserCreditsListener();
      updateUserCreditsUI();
    } else {
...
function loadAdminImagesRealtime() {
  // Recupera due container: uno per le immagini 1-8 (in alto) e uno per le immagini 9-12 (in basso)
    ...
    if (doc.exists) {
      ...
      for (let i = 1; i <= 12; i++) {
        ...
        if (url) {
          ...
          img.alt = `Immagine ${i}`;
          a.appendChild(img);
          // Immagini 1-8 in alto, 9-12 in basso
          if (i <= 8) {
            containerTop.appendChild(a);
          } else {
            containerBottom.appendChild(a);
          }
        }
      }
      if (currentUser === adminUsername) {
      ...
      // Inizializzazione con 12 immagini
      const initialData = {};
      for (let i = 1; i <= 12; i++) {
...
function resetAllReservations() {
  const today = getTodayDate(); // Data di oggi nel formato YYYY-MM-DD
  ...
  db.collection("reservations")
    ...
    .then(snapshot => {
      ...
      snapshot.forEach(doc => {
        const data = doc.data();
        const reservationDate = data.date; // Data della prenotazione

        if (reservationDate < today && data.user !== adminUsername) {
          // Cancella solo le prenotazioni di giorni passati (non future)
    ...
    .then(() => {
      showNotification("Prenotazioni passate cancellate con successo.");
      loadReservationsFromFirestore();
    })
    .catch(err => {
      console.error("Errore durante la cancellazione delle prenotazioni passate:", err);
