...
// Utenti statici – aggiorna l'elenco come preferisci
let staticUsers = {
  "admin": adminPassword,
  "user001": "jcne",
  "user002": "hoyw",
 ...
 ***********************/
// Carica le prenotazioni per la data selezionata (currentReservationDate)
// ATTENZIONE: il campo "date" DEVE essere salvato come stringa (es. "2025-02-26")
  ...
  db.collection("reservations")
    ...
    .then(querySnapshot => {
      querySnapshot.forEach(doc => {
        const data = doc.data();
        console.log("Documento letto:", data);  // DEBUG: stampa ogni documento
        const { field, time, user } = data;
        if (!field || !time || !user) {
          console.warn("Documento malformato:", data);
          return;
        }
...
function listenRealtimeForDate() {
  ...
  db.collection("reservations")
    .where("date", "==", date)
    .onSnapshot(snapshot => {
      // Resetta l'oggetto prenotazioni
      reservations = { "BeachVolley": {}, "Calcio": {}, "Polivalente": {} };
 ...
 ***********************/
// (Le funzioni per limite prenotazioni e gestione crediti rimangono invariate)
...
// Carica le raccolte per debug
const collectionsToLoad = ["reservations", "users", "admin"];
...
// Funzione che carica le raccolte specificate e stampa in console i documenti
...
// Richiama la funzione all'avvio (puoi richiamarla anche dopo il login o in base alle tue esigenze)
document.addEventListener('DOMContentLoaded', () => {
  loadAllCollections();
});
