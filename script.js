...
function loadAdminImagesRealtime() {
  db.collection("admin").doc("images").onSnapshot(doc => {
    ...
    if (doc.exists) {
      ...
      // Inserisce il blocco con il messaggio "CODICE SCONTO"
      const spacer = document.createElement("div");
      spacer.className = "codice-sconto";
      spacer.innerText = "Codice sconto: CAMPI3%";
      container.appendChild(spacer);
      
      // Carica immagini 9 e 10
      for (let i = 9; i <= 10; i++) {
        const urlField = `image${i}URL`;
        const linkField = `image${i}Link`;
        const url = data[urlField] || "";
